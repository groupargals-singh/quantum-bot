...
<body>
    <div class="header">
        ...
        {% else %}
        <div class="status-badge">SWARM ACTIVE 🟢</div>
        {% endif %}
    </div>
...
def dashboard():
    ...
    return render_template_string(HTML_DASHBOARD, perf=perf, snaps=multi_snapshots, is_paused=system_state.is_paused)
...
def start_phase7_engine():
    print("🚀 [QUANTUM BOT] Launching Phase 7 Interactive Command Swarm...")
    command_center.start_polling()
    time.sleep(1)
    l2_engine.start()

if __name__ == "__main__":
    engine_thread = threading.Thread(target=start_phase7_engine, daemon=True)
    engine_thread.start()
